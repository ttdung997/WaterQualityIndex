...
class ANFIS:
    ...
    def plotResults(self):
        if self.trainingType == 'Not trained yet':
            ...
            mseValue = np.mean(np.abs((self.fittedValues) - self.Y))
            print("MAE error!!!")
            print(mseValue)
            print(mseValue/max(self.Y))
            predict_label = [1 if i > 60 else 0 for i in self.fittedValues]
            real_label = [1 if i > 60 else 0 for i in self.Y]
